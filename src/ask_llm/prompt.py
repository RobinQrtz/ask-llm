...
def apply_askllm_prompt(example, tokenizer, language, max_length=512):
    """
    ...
    document_context = "###\n" + text_extract + "\n###\n"
    prompt_second = f"""Does the previous paragraph demarcated within ### and ###
    contain informative signal for pre-training a large-language model for {language}?
    ...
    text = document_context + prompt_second
    # text = prompt_first + document_context
...
def askllm_prompt_prefix_length(tokenizer, tokenizer_type="llama"):
    prompt_first = """Does the following paragraph demarcated within ### and ###
    contain informative signal for pre-training a large-language model in Swedish?
    An informative datapoint should be well-formatted, contain some
    usable knowledge of the world, and not have any autogenerated or autotranslated marketing, etc. content.
